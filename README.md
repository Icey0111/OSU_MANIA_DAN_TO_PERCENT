...
系统按完整段位统计，`Low / Mid / High` 只作为该段位内的子等级，不会被当作跨段位差异。当至少有 20 票、领先段位至少有 12 票且占总票数 60%，并以至少 2:1 领先第二名时，该段位会成为社区基准：相邻一个完整段位仍可投票，但会标记为“置信度低，请谨慎参考”；相差两个或更多完整段位的投票会被客户端和服务端拒绝。游戏内仅显示基准段位，浏览器页面仍会展示基准及相邻段位的详细数据。
